...
const AppImage = memo(function AppImage({
  ...
  // Keep the displayed image in sync when the parent changes `src`.
  // Without this, `useState(src)` only seeds the very first value and the
  // image gets stuck on a stale URL when a prop changes (e.g. switching events).
  useEffect(() => {
    setImageSrc(src);
    setHasError(false);
    setIsLoading(true);
